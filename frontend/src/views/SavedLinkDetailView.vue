...
import { getTutorialDocExcerpt, getTutorialDocPlainText, isTutorialDoc } from '../utils/tutorialDocs'
...
const record = ref<SavedLink | null>(null)
const allLinks = ref<SavedLink[]>([])
const previewDialogVisible = ref(false)
const previewDialogUrl = ref('')
const previewDialogTitle = ref('')

const currentIsTutorial = computed(() => Boolean(record.value && isTutorialDoc(record.value)))
const detailHtml = computed(() =>
  currentIsTutorial.value ? renderSavedLinkMarkdown(record.value?.description ?? '') : '',
)
...
const authorName = computed(() => {
  ...
  if (currentUser && currentUser.id === currentRecord.author_user_id) {
    return currentUser.display_name?.trim() || currentUser.username
  }

  return currentRecord.author_username
})
...
const authorAvatarUrl = computed(() => {
  ...
  return currentRecord.author_avatar_url
})
...
const categoryLabel = computed(() => getReaderCategoryLabel(record.value?.category))
const primaryUrl = computed(() => getPrimaryUrlFromSavedLink(record.value))
const displayDescription = computed(() => getDisplayDescription(record.value))
const contentSegments = computed(() => getDescriptionSegments(displayDescription.value))
const plainText = computed(() => getTutorialDocPlainText(displayDescription.value))
const readingMinutes = computed(() => estimateReadingMinutes(plainText.value))
const contentLength = computed(() => plainText.value.replace(/\s+/g, '').trim().length)
const contentLinkCount = computed(() => {
  ...
  const urls = new Set(extractUrlsFromText(currentRecord.description))
  const normalizedUrl = currentRecord.url?.trim() ?? ''
...
const authorEntryCount = computed(() => {
  ...
  return allLinks.value.filter((item) => item.author_user_id === currentRecord.author_user_id).length
})
const relatedLinks = computed(() => getReaderRelatedLinks(record.value, allLinks.value, 5))
const categoryStats = computed(() =>
  getReaderCategoryStats(allLinks.value, currentIsTutorial.value ? true : false, 8),
)
const recordExcerpt = computed(() => {
  const excerpt = getTutorialDocExcerpt(displayDescription.value, 120)
  const title = record.value?.title?.trim() ?? ''
  return excerpt && excerpt !== title ? excerpt : ''
})
const shouldShowGallery = computed(() => {
  const currentRecord = record.value
  if (!currentRecord?.images.length) {
    return false
  }

  if (currentIsTutorial.value && hasInlineImage(currentRecord.description)) {
    return false
  }
...
const canEdit = computed(() => {
  ...
  return authStore.canWrite('links') && (currentUser.role === 'superadmin' || currentUser.id === currentRecord.author_user_id)
})
...
function getDescriptionSegments(description: string | null | undefined): DescriptionSegment[] {
  ...
  for (const url of urls) {
    const startIndex = source.indexOf(url, cursor)
    if (startIndex === -1) {
      continue
    }

    if (startIndex > cursor) {
      segments.push({ type: 'text', value: source.slice(cursor, startIndex) })
    }
...
function hasInlineImage(value: string | null | undefined) {
  const normalized = String(value ?? '')
  return /!\[[^\]]*\]\([^)]+\)|<img\b/i.test(normalized)
}

function getDisplayDescription(link: SavedLink | null) {
  if (!link?.description) {
    return ''
  }

  return currentIsTutorial.value ? getTutorialDocPlainText(link.description) : link.description
}
...
async function loadRecord() {
  ...
  try {
    ...
    const target = data.find((item) => item.id === recordId)
    if (!target) {
...
function editCurrent() {
  ...
  if (currentIsTutorial.value) {
    const target = router.resolve({ name: 'tutorial-docs-editor-edit', params: { id: record.value.id } })
    ...
    void router.push({ name: 'tutorial-docs-editor-edit', params: { id: record.value.id } })
    return
  }
...
function openPrimaryImage() {
  ...
  const primaryImage = currentRecord.images[0]
  openImage(primaryImage.url, primaryImage.name || `${currentRecord.title} - 图片`)
}
...
<template>
  <div class="page-stack">
          ...
          <header class="reader-page-head">
            <div class="reader-page-head__main">
              <div class="reader-page-head__crumb">
                ...
                <span>链接广场</span>
                <span>{{ currentIsTutorial ? '文章详情' : '帖子详情' }}</span>
              </div>

              <strong class="reader-page-head__title">{{ record.title }}</strong>
            </div>

            <div class="reader-page-head__actions">
              <el-button v-if="primaryUrl" plain @click="openExternal">打开原链</el-button>
              <el-button v-if="canEdit" plain @click="editCurrent">编辑</el-button>
            </div>
          </header>
          ...
          <div class="reader-layout">
            ...
            <div class="reader-main">
              <section class="reader-post-card">
                <div class="reader-post-card__head">
                  <div class="reader-post-card__headline">
                    <div class="reader-post-card__labels">
                      ...
                      <span class="reader-post-card__type">
                        {{ currentIsTutorial ? '文章' : '帖子' }}
                      </span>
                    </div>
                    ...
                    <div class="reader-post-card__actions">
                      ...
                      <el-button v-if="canEdit" text @click="editCurrent">编辑</el-button>
                    </div>
                  </div>

                  <h1 class="reader-post-card__title">{{ record.title }}</h1>
                ...
                <div v-if="recordExcerpt" class="reader-post-card__summary">
                  {{ recordExcerpt }}
                </div>
                ...
                <div class="reader-post-card__section">
                  <div class="reader-post-card__section-title">
                    <span></span>
                    <strong>{{ currentIsTutorial ? '正文内容' : '帖子内容' }}</strong>
                  </div>

                  <div class="reader-post-card__content">
                    <article
                      v-if="currentIsTutorial && detailHtml"
                      class="reader-content"
                      v-html="detailHtml"
                    ></article>

                    <article v-else-if="currentIsTutorial" class="reader-empty">
                      <strong>暂无正文内容</strong>
                      <span>这篇文章还没有填入正文。</span>
                    </article>

                    <article v-else class="reader-content reader-content--plain">
                      <div v-if="displayDescription" class="reader-content__text">
                      ...
                      <div v-else class="reader-empty">
                        <strong>暂无正文内容</strong>
                        <span>当前内容还没有补充更完整的说明。</span>
                      </div>
                    </article>
                  </div>
                </div>

                <div v-if="shouldShowGallery" class="reader-post-card__section reader-post-card__section--gallery">
                  <div class="reader-post-card__section-title">
                    <span></span>
                    <strong>内容配图</strong>
                  </div>
                  ...
                  <div
                    class="reader-gallery"
                    :class="{
                      'reader-gallery--single': record.images.length === 1,
                      'reader-gallery--double': record.images.length === 2,
                    }"
                  >
                    <button
                      ...
                      :key="`${record.id}-${index}`"
                      type="button"
                      class="reader-gallery__item"
                      @click="openImage(image.url, `${record.title} - 图片 ${index + 1}`)"
                    >
                      <img :src="image.url" :alt="image.name || `${record.title} 配图 ${index + 1}`" />
                    </button>
                  </div>
                </div>
              </section>
                ...
                <section class="reader-sidecard reader-sidecard--author">
                  <div class="reader-author-card">
                    ...
                    <div class="reader-author-card__stats">
                      ...
                      <div>
                        <strong>{{ record.images.length }}</strong>
                        <span>配图数量</span>
                      </div>
                    </div>

                    <div class="reader-author-card__info">
                      <span>发布时间：{{ formatDateTime(record.created_at) }}</span>
                    ...
                    <div class="reader-author-card__actions">
                      ...
                      <el-button v-if="canEdit" plain @click="editCurrent">编辑内容</el-button>
                    </div>
                  </div>
                </section>
                  ...
                  <div v-if="relatedLinks.length" class="reader-related-list">
                    ...
                    >
                      ...
                      <span class="reader-related-item__meta">
                        {{ getReaderCategoryLabel(item.category) }} · {{ formatDateTime(item.updated_at) }}
                      </span>
                    </button>
                  </div>
    ...
    <el-dialog
      ...
      :title="previewDialogTitle || '图片预览'"
      width="960px"
      destroy-on-close
    >
      <div class="reader-image-dialog">
        <img v-if="previewDialogUrl" :src="previewDialogUrl" :alt="previewDialogTitle || '图片预览'" />
      </div>
    </el-dialog>
  </div>
</template>
...
.reader-post-card__section--gallery {
  margin-top: 30px;
}
...
.reader-content__link {
  color: #1677ff;
  text-decoration: none;
}

.reader-content__link:hover {
...
.reader-gallery--single {
  grid-template-columns: minmax(0, 420px);
}

.reader-gallery--double {
  grid-template-columns: repeat(2, minmax(0, 1fr));
}

.reader-gallery__item {
  display: block;
  aspect-ratio: 4 / 3;
  overflow: hidden;
  padding: 0;

...
<template><IonPage><IonContent :fullscreen="true"><main class="mine-page">
  <section class="mine-hero">
    ...
    <div class="mine-identity"><button class="mine-avatar" type="button" aria-label="编辑头像和姓名" @click="openProfile"><img v-if="avatarUrl" :src="avatarUrl" alt="头像"><span v-else class="mine-avatar-fallback">{{ displayName.slice(0,2) }}</span><i><IonIcon :icon="cameraOutline" /></i></button><div><h1>{{ displayName }}</h1><div class="mine-account-meta"><em>{{ roleLabel }}</em><span class="mine-id-badge"><strong class="mine-id-label">ID:</strong><b>{{ session.user?.username||'--' }}</b></span></div></div></div>
...
.profile-editor>header{height:58px;padding:0 14px;display:grid;grid-template-columns:64px 1fr 64px;align-items:center;border-bottom:1px solid var(--app-line);background:var(--app-card)}

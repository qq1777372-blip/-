...
import { IonContent, IonIcon, IonModal, IonPage, alertController, toastController } from '@ionic/vue'
import { chevronBackOutline, chevronForwardOutline, closeOutline, copyOutline, createOutline, openOutline, trashOutline } from 'ionicons/icons'
...
const previewImages=computed(()=>{
  const images=[
    ...contentBlocks.value.filter(block=>block.type==='image').map(block=>({url:block.src,alt:block.alt})),
    ...remainingImages.value.map(image=>({url:image.url,alt:image.name||item.value?.title||'帖子图片'})),
  ]
  return images.filter((image,index)=>images.findIndex(candidate=>normalizePath(candidate.url)===normalizePath(image.url))===index)
})
const previewIndex=ref(-1)
const previewImage=computed(()=>previewImages.value[previewIndex.value]||null)

function openImage(url?:string){if(!url)return;const index=previewImages.value.findIndex(image=>normalizePath(image.url)===normalizePath(url));previewIndex.value=index<0?0:index}
function closeImage(){previewIndex.value=-1}
function moveImage(step:number){const total=previewImages.value.length;if(total>1)previewIndex.value=(previewIndex.value+step+total)%total}
...
<template>
  <IonPage>
        ...
        <article class="link-reader-post">
          ...
          <div v-if="contentBlocks.length" class="link-reader-content">
            <template v-for="(block,index) in contentBlocks" :key="index">
              ...
              <figure v-else :class="`align-${block.align}`"><button class="link-reader-image-button" type="button" aria-label="查看大图" @click="openImage(block.src)"><img :src="block.src" :alt="block.alt"></button></figure>
            </template>
          </div>
          ...
          <section v-if="remainingImages.length" class="link-reader-gallery"><button v-for="image in remainingImages" :key="image.storage_name" class="link-reader-image-button" type="button" aria-label="查看大图" @click="openImage(image.url)"><img :src="image.url" :alt="image.name||item.title"></button></section>
    ...
    <IonModal :is-open="Boolean(previewImage)" css-class="link-image-preview-modal" @did-dismiss="closeImage">
      <div class="link-image-preview" @click.self="closeImage">
        <button class="link-image-preview__close" type="button" aria-label="关闭图片预览" @click="closeImage"><IonIcon :icon="closeOutline" /></button>
        <button v-if="previewImages.length>1" class="link-image-preview__nav link-image-preview__nav--prev" type="button" aria-label="上一张" @click="moveImage(-1)"><IonIcon :icon="chevronBackOutline" /></button>
        <img v-if="previewImage" :src="previewImage.url" :alt="previewImage.alt">
        <button v-if="previewImages.length>1" class="link-image-preview__nav link-image-preview__nav--next" type="button" aria-label="下一张" @click="moveImage(1)"><IonIcon :icon="chevronForwardOutline" /></button>
        <span v-if="previewImages.length>1" class="link-image-preview__count">{{ previewIndex+1 }} / {{ previewImages.length }}</span>
      </div>
    </IonModal>
  </IonPage>
</template>
...
.link-image-preview-modal{--width:100%;--height:100%;--border-radius:0;--background:#050505}.link-image-preview{position:relative;width:100%;height:100%;display:grid;place-items:center;padding:56px 52px;background:#050505}.link-image-preview>img{display:block;max-width:100%;max-height:100%;object-fit:contain}.link-image-preview button{position:absolute;z-index:2;width:44px;height:44px;padding:0;border:0;border-radius:50%;display:grid;place-items:center;color:#fff;background:#1f1f1fcc}.link-image-preview button ion-icon{font-size:27px}.link-image-preview__close{top:calc(env(safe-area-inset-top,0px) + 12px);right:14px}.link-image-preview__nav{top:50%;transform:translateY(-50%)}.link-image-preview__nav--prev{left:8px}.link-image-preview__nav--next{right:8px}.link-image-preview__count{position:absolute;bottom:calc(env(safe-area-inset-bottom,0px) + 16px);left:50%;transform:translateX(-50%);padding:5px 10px;border-radius:999px;color:#fff;background:#1f1f1fcc;font-size:12px}
